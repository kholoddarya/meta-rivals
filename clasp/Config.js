...
const SETTINGS_PAGES = [
  ...
  {
    key: 'compositions',
    name: '🎯 Раскладки и обязательные персонажи',
    func: 'showMustHaveHeroes',
  },
...
function validateSettings(settings) {
  ...
  if (
    settings.TEAM_SIZE === undefined ||
    settings.TEAM_SIZE === null ||
    settings.TEAM_SIZE < 1 ||
    settings.TEAM_SIZE > 10
  ) {
    errors.push(`TEAM_SIZE должен быть от 1 до 10, получено: ${settings.TEAM_SIZE}`)
  }

  if (
    settings.MAX_RESULTS === undefined ||
  ...
  if (
    settings.MIN_SYNERGIES === undefined ||
    settings.MIN_SYNERGIES === null ||
