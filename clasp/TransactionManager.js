...
class SettingsTransaction {
  ...
  begin() {
    this.transactionId = 'tx_' + new Date().getTime() + '_' + Math.random().toString(36).substr(2, 9);
...
function saveSettingsTransactional(settings, silent = false) {
  ...
  try {
    ...
    if (validationErrors.length > 0) {
      ...
      Logger.log(`Текущие значения: TEAM_SIZE=${settings.TEAM_SIZE}, MAX_RESULTS=${settings.MAX_RESULTS}, MIN_SYNERGIES=${settings.MIN_SYNERGIES}`);
      tx.rollback();
      return false;
    }
