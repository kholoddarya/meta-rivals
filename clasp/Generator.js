...
function runGenerationAsync() {
  ...
  try {
    ...
    const outputTeamSize = settings.GENERATION_MODE === "counter" 
      ? (settings.COUNTER_ROLE_COMPOSITION.sup + settings.COUNTER_ROLE_COMPOSITION.dps + settings.COUNTER_ROLE_COMPOSITION.tnk)
      : settings.TEAM_SIZE;
    
    // ========== ЧТЕНИЕ ДАННЫХ ==========
    const matrixSheet = ss.getSheetByName(settings.MATRIX_SHEET_NAME);
    if (!matrixSheet) {
      return { success: false, error: `Лист "${settings.MATRIX_SHEET_NAME}" не найден.`, fixable: true };
    }
    
    const matrixData = matrixSheet.getDataRange().getValues();
    const matrixRowStart = getMatrixRowStart(matrixData);
    const rowHeaders = matrixData.slice(matrixRowStart).map(row => row[0]).filter(n => n).map(n => n.toString().trim());
    ...
    if (settings.GENERATION_MODE === "counter" && settings.ENEMY_COMPOSITION.length > 0) {
      ...
      const enemyValidation = validateEnemyComposition(settings.ENEMY_COMPOSITION, rowHeaders, classesMapForEnemies);
    ...
    if (!rolesMap) {
      return { success: false, error: `Лист ролей "${settings.ROLES_SHEET_NAME}" не найден.`, fixable: true };
    }
    
    // ========== ЧТЕНИЕ ТИРОВ ==========
    const TIER_SCORES = { 
      "S": settings.TIER_S, "A": settings.TIER_A, "B": settings.TIER_B, 
      "C": settings.TIER_C, "D": settings.TIER_D 
    };
    ...
    const GRADE_SCORES = { 
      "S": settings.GRADE_S, "SA": settings.GRADE_SA, 
      "A": settings.GRADE_A, "B": settings.GRADE_B 
    };
    
    const { graph, soloCapableChars } = buildSynergyGraph(
      rowHeaders, colHeaders, matrixData, matrixRowStart, 
      GRADE_SCORES, settings.USE_SOLO_SYNERGIES
    );
    ...
    const counterPickCache = precomputeCounterPicks(
      rowHeaders, classesMap, enemyIndices, enemyClassesMap, settings
    );
    ...
    const context = {
      settings, rowHeaders, rolesMap, graph, soloCapableChars,
      heroTiersMap, classesMap, mustHaveIndices, enemyIndices,
      enemyClassesMap, counterPickCache, GRADE_SCORES
    };
        ...
        for (let di = 0; di < dpsComb.length; di++) {
          for (let ti = 0; ti < tnkComb.length; ti++) {
            ...
            if (topTeams.length >= settings.MAX_RESULTS) {
              ...
              const upperBound = calculateUpperBound(team, soloCapableChars, GRADE_SCORES) + comp.weight;
...
function writeResultsToSheet(ss, topTeams, resultSheetName, outputTeamSize, settings, duration) {
  ...
  if (!targetSheet) {
    ...
    targetSheet.clear();
    try { targetSheet.showColumns(1, targetSheet.getMaxColumns()); } catch (e) {}
  }
  
  let headerRowOffset = 0;
  if (settings.GENERATION_MODE === "counter") {
    const enemyNames = settings.ENEMY_COMPOSITION.join(", ");
    targetSheet.getRange(1, 1).setValue(`⚔️ Против: ${enemyNames}`)
      .setFontWeight("bold").setFontSize(12).setFontColor("#d32f2f");
  ...
  if (settings.USE_COUNTER_PICKS && settings.GENERATION_MODE === "counter") headers.push("Контрпики");
  ...
  if (topTeams.length > 0) {
    ...
    for (let r = 0; r < topTeams.length; r++) {
      ...
      if (settings.USE_COUNTER_PICKS && settings.GENERATION_MODE === "counter") row.push(t.counterScore);
    ...
    targetSheet.getRange(headerRow, 1, 1, headers.length).setFontWeight("bold").setBackground("#f3f3f3");
...
function quickPickTeams(enemies, myHeroes, neededRoles, options, bannedHeroes, starredEnemies) {
  try {
    ...
    Logger.log(`=== Быстрый подбор === maxResults: ${maxResults}, ⭐ Звёзд: ${starredSet.size}, FLX: ${neededRoles.filter(r => r === 'flx').length}`);
    ...
    const { graph, soloCapableChars } = buildSynergyGraph(
      rowHeaders, colHeaders, matrixData, matrixRowStart, 
      GRADE_SCORES, opts.useSoloSynergies
    );
    ...
    const counterPickCache = (opts.useCounterPicks && settings.USE_COUNTER_PICKS && enemyIndices.length > 0)
    ...
    const totalTeamSize = fixedRoles.sup + fixedRoles.dps + fixedRoles.tnk + flxCount + myHeroIndices.length;
    ...
    const context = {
      settings: localSettings, rowHeaders, rolesMap, graph, soloCapableChars,
      heroTiersMap, classesMap, mustHaveIndices: [], enemyIndices,
    ...
    for (const dist of flxDistributions) {
      ...
      if (totalRoles.sup > charactersByRole.sup.length ||
          ...
          for (let ti = 0; ti < tnkComb.length; ti++) {
            ...
            if (res) {
              ...
              res.addedMembers = uniqueTeam
                .filter(function(idx) { return !mySet.has(idx); })
                .map(function(idx) { return rowHeaders[idx]; });
    ...
    const duration = ((new Date().getTime() - startTime) / 1000).toFixed(2);
    Logger.log(`✅ Подбор за ${duration}с, итераций: ${totalIterations}, отсечено: ${skippedByBound}, в топ: ${topTeams.length}`);
    ...
    const weightContext = { 
      settings: localSettings, rolesMap, classesMap, heroTiersMap, 
      enemyIndices, enemyClassesMap, rowHeaders, 
      starredEnemyIndices: starredEnemyIndices
    };
    ...
    if (topK.length > 0) {
      debugTeamSynergies(topK[0].members.map(name => rowHeaders.indexOf(name)), context);
    }
...
function calculateHeroIndividualWeight(heroName, context) {
  const { settings, rolesMap, classesMap, heroTiersMap, 
          enemyIndices, enemyClassesMap, rowHeaders } = context;
  ...
  if (classesMap && classesMap[heroName]) {
    ...
    if (settings.USE_CLASS_WEIGHTS && myClass && settings.CLASS_WEIGHTS && settings.CLASS_WEIGHTS[myClass]) {
      weight += settings.CLASS_WEIGHTS[myClass];
    }
    if (settings.USE_ANTI_ROLE_WEIGHTS && myCounter && settings.ANTI_ROLE_WEIGHTS && settings.ANTI_ROLE_WEIGHTS[myCounter]) {
    ...
    if (settings.USE_COUNTER_PICKS && enemyIndices.length > 0) {
      for (const enemyIdx of enemyIndices) {
        ...
        const isStarred = context.starredEnemyIndices && context.starredEnemyIndices.indexOf(enemyIdx) !== -1;
...
function calculateQuickUpperBound(team, context) {
  const { settings, heroTiersMap, rowHeaders, rolesMap, classesMap, GRADE_SCORES, soloCapableChars, enemyIndices, enemyClassesMap } = context;
  ...
  if (settings.ROLE_WEIGHTS) {
    maxPossible += (roleCounts.sup * (settings.ROLE_WEIGHTS.sup || 0)) +
...
function checkSynergyRequirements(team, teamData, context) {
  ...
  if (isSingleCounter || !settings.MUST_HAVE_SYNERGY || mustHaveIndices.length === 0 || !settings.USE_SYNERGIES) {
    return true;
  }
...
function evaluateTeam(team, composition, compositionWeight, context) {
  ...
  const finalComposition = finalRoleCounts.sup + '-' + finalRoleCounts.dps + '-' + finalRoleCounts.tnk;
  ...
  const synergyCheck = (settings.USE_SYNERGIES && team.length > 1) 
    ? scores.synergyCount >= settings.MIN_SYNERGIES 
    : true;
  
  if (synergyCheck) {
    return {
      members: team.map(function(idx) { return context.rowHeaders[idx]; }),
      roles: team.map(function(idx) { return context.rolesMap[context.rowHeaders[idx]]; }),
...
function collectTeamData(team, mySet, context) {
  const { rowHeaders, rolesMap, classesMap, heroTiersMap, settings, enemyIndices, enemyClassesMap } = context;
...
function calculateAllScores(teamData, context) {
  const { settings, rowHeaders, graph, soloCapableChars, heroTiersMap, classesMap, 
          enemyIndices, enemyClassesMap, myHeroIndices, starredEnemyIndices } = context;
  
  const scores = {
    synergy: 0, synergyCount: 0, minScore: Infinity,
    tier: 0, counter: 0, role: 0, heavy: 0
  };
    ...
    if (candidates.length > 0) {
      ...
      if (best.hasRealPartner) {
        ...
        if (!processedPairs.has(pairKey)) {
          ...
          if ((isProposed || partnerIsProposed) && 
              best.grade && 
  ...
  // Веса ролей
  if (settings.ROLE_WEIGHTS) {
    scores.role = (teamData.roleCounts.sup * (settings.ROLE_WEIGHTS.sup || 0)) +
    ...
    if (scores.role > 0) {
      ...
      teamData.details.role.push('Состав: ' + parts.join(' ') + ' = ' + scores.role + ' очков за роли');
    }
  }
  ...
  if (settings.USE_HEAVY_CLASS && settings.HEAVY_CLASS_BONUS > 0 && team.length > 1) {
    ...
    if (classValues.length > 0) {
      ...
      if (ratio >= 0.67) {
        ...
        teamData.details.heavy.push('Heavy ' + dominant + ': ' + maxCount + '/' + team.length + ' героев (' + Math.round(ratio * 100) + '%)');
      }
    }
  }
...
function buildTeamResult(team, composition, compositionWeight, teamData, scores, totalScore, context) {
...
function formatTeamDetails(teamData) {
  ...
  function makeBlock(id, icon, title, items, itemColorClass) {
    ...
    if (id === 'starcounters') {
      ...
      itemsHtml = items.map(function(d) {
        var text;
        ...
        return '<div class="detail-row"><span class="detail-text detail-star">' + text + '</span></div>';
      }).join('');
    } else {
      itemsHtml = items.map(function(d) {
        return '<div class="detail-row">' +
          '<span class="detail-text ' + (itemColorClass || '') + '">' + d + '</span>' +
          '</div>';
      }).join('');
    }
    
    return '<div class="detail-block expanded" id="' + id + '">' +
      '<div class="detail-block-header" onclick="toggleDetailBlock(event, this)">' +
        '<span><span class="block-icon">' + icon + '</span>' + title + ' (' + items.length + ')</span>' +
        '<span class="block-toggle">▶</span>' +
      '</div>' +
      '<div class="detail-block-content">' + itemsHtml + '</div>' +
      '</div>';
  }
  ...
  html += makeBlock('starcounters', '🌟', 'Контр-ключевые (×2)', teamData.starCounterDetails, 'detail-positive'); 
  ...
  if (html === '') {
    html = '<div style="padding:8px;color:#666;font-style:italic;font-size:12px;">Нет выраженных преимуществ</div>';
  }
...
function formatGroupDetails(group) {
  ...
  function makeBlock(id, icon, title, items, itemColorClass) {
    ...
    const itemsHtml = items.map(function(d) {
      return '<div class="detail-row">' +
        '<span class="detail-text ' + (itemColorClass || '') + '">' + d + '</span>' +
        '</div>';
    }).join('');
    
    return '<div class="detail-block expanded">' +
      '<div class="detail-block-header" onclick="toggleDetailBlock(event, this)">' +
        '<span><span class="block-icon">' + icon + '</span>' + title + ' (' + items.length + ')</span>' +
        '<span class="block-toggle">▶</span>' +
      '</div>' +
...
function evaluateSpecificTeam(heroNames, myHeroes, enemies, options, bannedHeroes, starredEnemies) {
  try {
    ...
    const matrixData = matrixSheet.getDataRange().getValues();
    const matrixRowStart = getMatrixRowStart(matrixData);
    const rowHeaders = matrixData.slice(matrixRowStart)
      .map(r => r[0]).filter(n => n).map(n => n.toString().trim());
    const headerRowIndex = matrixRowStart > 0 ? matrixRowStart - 1 : 0;
    const colHeaders = matrixData[headerRowIndex].slice(1).filter(h => h);
    ...
    const context = {
      ...
      heroTiersMap, classesMap, mustHaveIndices: [], enemyIndices,
      starredEnemyIndices: starredEnemyIndices,
      enemyClassesMap: enemyClassesMap, counterPickCache: {}, GRADE_SCORES,
      myHeroIndices: myHeroIndices
    };
    ...
    const mySet = new Set(myHeroIndices);
    result.addedMembers = teamIndices
