...
function getMatrixRowStart(matrixData) {
  ...
  for (let i = 0; i < Math.min(10, matrixData.length); i++) {
    ...
    if (firstCell && typeof firstCell === 'string') {
      ...
      if (
        lower.includes('персонаж') ||
        lower.includes('character') ||
        lower.includes('имя') ||
        lower.includes('name') ||
        lower.includes('матрица') ||
        lower.includes('matrix') ||
        lower.includes('герой') ||
        lower.includes('hero')
      ) {
...
function readRolesMap(sheetName) {
  ...
  if (data.length > 0 && data[0][0]) {
    ...
    if (
      firstCell.includes('персонаж') ||
      firstCell.includes('имя') ||
      firstCell.includes('name') ||
      firstCell.includes('hero') ||
      firstCell.includes('роль') ||
      firstCell.includes('role')
    ) {
  ...
  if (Object.keys(rolesMap).length > 0) {
    Logger.log(
      `  Первый герой: "${Object.keys(rolesMap)[0]}" (${rolesMap[Object.keys(rolesMap)[0]]})`
    )
...
function readClassesMap(sheetName) {
  ...
  if (Object.keys(classesMap).length > 0) {
    Logger.log(
      `  Первый герой: "${Object.keys(classesMap)[0]}" (${classesMap[Object.keys(classesMap)[0]].class})`
    )
    Logger.log(
      `  Последний герой: "${Object.keys(classesMap)[Object.keys(classesMap).length - 1]}"`
    )
  }
...
function readHeroTiersMap(sheetName, tierScores) {
  ...
  if (data.length > 0 && data[0][0]) {
    ...
    if (
      ...
      firstCell.includes('name') ||
      firstCell.includes('hero') ||
      firstCell.includes('тир') ||
      firstCell.includes('tier')
    ) {
      // Есть заголовок — начинаем со второй строки
      startRow = 1
      Logger.log(`  Заголовок найден в строке 0, начинаем с ${startRow}`)
  ...
  if (Object.keys(tiersMap).length > 0) {
    Logger.log(
      `  Первый герой: "${Object.keys(tiersMap)[0]}" (${tiersMap[Object.keys(tiersMap)[0]].tier})`
    )
  }
  ...
  for (let i = 0; i < rowHeaders.length; i++) {
    ...
    for (let j = 0; j < colHeaders.length; j++) {
      ...
      if (cellIndex >= row.length) {
        Logger.log(
          `⚠️ Ячейка ${cellIndex} для ${charName}→${partnerName} выходит за пределы строки`
        )
        continue
      }
...
function getHeroesData() {
  ...
  Logger.log(
    `✅ Найдено ${heroNames.length} имён героев: ${heroNames.slice(0, 10).join(', ')}${heroNames.length > 10 ? '...' : ''}`
  )
  ...
  if (heroNames.length > 0) {
    ...
    Logger.log(
      `  - Есть в rolesMap: ${rolesMap[firstHero] ? 'ДА (' + rolesMap[firstHero] + ')' : 'НЕТ'}`
    )
