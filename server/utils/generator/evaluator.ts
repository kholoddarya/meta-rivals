...
export function evaluateTeam(
  ...
  for (const myIdx of myHeroIndices) {
    if (!team.includes(myIdx)) {
      return null // Команда невалидна, если в ней нет всех моих героев
    }
  }
  ...
  for (const idx of team) {
    ...
    const role: RoleType =
      rawRole === 'sup' || rawRole === 'dps' || rawRole === 'tnk' ? rawRole : 'dps'
    ...
    if (classData) {
      ...
      if (isProposed) {
        ...
        details.class.push(`${name} — ${parts.join(' / ') || 'класс не указан'}`)
      }
        ...
        for (const enemyIdx of enemyIndices) {
          ...
          if (myCounter && classCounters[myCounter] === enemyClass) {
            ...
            if (isProposed) {
              ...
              if (isStarred) details.starCounters.push(`${name} → контрит ключевого ${enemyName}`)
            }
      ...
      if (options.useAntiRoleWeights && myCounter && antiRoleWeights[myCounter]) {
  ...
  if (roleScore > 0) {
    ...
    details.role.push(`Состав: ${parts.join(' ')} = ${roleScore} очков за роли`)
  }
  ...
  return {
    ...
    addedMembers: team.filter((idx) => !mySet.has(idx)).map((idx) => rowHeaders[idx]),
